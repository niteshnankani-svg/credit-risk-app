...
if st.session_state.request_count >= RATE_LIMIT:
    st.error(f"⚠️ Rate limit reached. Maximum {RATE_LIMIT} predictions per session.")
    st.info("Clear your history below to reset.")
    st.stop()
else:
    remaining = RATE_LIMIT - st.session_state.request_count
    st.caption(f"Predictions remaining this session: {remaining} / {RATE_LIMIT}")
